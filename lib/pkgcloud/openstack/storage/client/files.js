 ...
 * @param callback
 */
exports.removeLargeFile = function (container, file, callback) {
  var containerName = container instanceof this.models.Container ? container.name : container,
      fileName = file instanceof this.models.File ? file.name : file;
 ...
 * @param {String|object}   options.container   the container to store the file in
 * @param {String}          options.remote      the file name for the new file
 * @param {String}          [options.start]     the first byte to start reading
 * @param {String}          [options.end]       the last byte to end reading
...
exports.download = function (options, callback) {
  ...
  if( options.start || options.end ){
    headers['Range'] = util.format('bytes=%s-%s', options.start || '0', options.end || '');
  }
...
exports.copy = function (options, callback) {
  ...
  if(options.sourceFile instanceof self.models.File){
    copyOptions.uri = options.sourceFile.fullPath;
  } else {
    copyOptions.path = options.sourceFile;
